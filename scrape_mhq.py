...
The site has no public API (server-rendered Django pages). The list page accepts
?country=FR&game_system=1&page_size=48&page=N. Each card gives name, date, game,
registrations. The detail page embeds a Leaflet marker with coordinates and the
address, plus the status and number of rounds. Detail pages are cached under
cache/mhq/ and re-fetched only for events that are not finished yet.
"""
...
def parse_list(page_html):
    cards = []
    for m in re.finditer(r'<a href="(https://miniheadquarters\.com/tournaments/individual/details/[^"]+)"(.*?)</a>', page_html, flags=re.S):
...
def main():
    ...
    def list_url(page):
        return f"{BASE}/tournaments/individual/?" + urllib.parse.urlencode({"country": a.country, "game_system": a.game, "page_size": 48, "page": page})

    first = fetch(list_url(1))
    if not first:
        raise SystemExit("cannot reach miniheadquarters")
    cards, total = parse_list(first)
    pages = -(-total // 48) if total else 1
    print(f"{total} tournaments, {pages} list pages", file=sys.stderr)
    for p in range(2, pages + 1):
        h = fetch(list_url(p))
        if h:
            more, _ = parse_list(h)
            cards.extend(more)
            print(f"  list page {p}/{pages}: {len(more)} cards", file=sys.stderr)
        time.sleep(0.3)
    by_slug = {c["slug"]: c for c in cards}
    print(f"{len(by_slug)} unique tournaments", file=sys.stderr)

    today = dt.date.today().isoformat()

    def get_detail(card):
        path = os.path.join(a.cache, card["slug"] + ".json")
        finished_cached = False
        if os.path.exists(path) and not a.refresh:
            with open(path, encoding="utf-8") as f:
                cached = json.load(f)
            finished_cached = (card["date"] or "") < today and cached.get("status") == "Finished"
            if finished_cached or (card["date"] or "9999") < "0000":
                return cached
            if (card["date"] or "") < today and "lat" in cached:
                return cached
        h = fetch(card["url"])
        if not h:
            return {}
    ...
    with cf.ThreadPoolExecutor(max_workers=a.workers) as ex:
        futs = {ex.submit(get_detail, c): c for c in by_slug.values()}
        for fut in cf.as_completed(futs):
            ...
            out.append({
                "source": "mhq", "id": f"mhq:{c['slug']}", "name": d.get("nameFull") or c["name"], "url": c["url"],
                ...
                "players": c["players"], "capacity": c["capacity"], "interested": c["interested"],
                "rounds": d.get("rounds"), "teamEvent": False,
                "status": d.get("status"),
    ...
    print(f"{len(out)} events, {missing} without coordinates", file=sys.stderr)
